with open('C:\\sqlite\\mysql\\code\\Code06\\singer1.csv', 'r') as inFp:
    with open('C:\\sqlite\\mysql\\code\\Code06\\newsinger2.csv', 'w') as outFp:
        header = inFp.readline()
        header = header.strip() # \n 제거
        header_list = header.split(',') # , 제거
        idx1 = header_list.index('아이디') #index(위치)-> 아이디0번째 이름1번째 키6번째 
        # print('idx1', idx1)
        idx2 = header_list.index('이름') #index(위치)-> 아이디0번째 이름1번째 키6번째 
        # print('idx2', idx2)
        idx3 = header_list.index('평균 키') #index(위치)-> 아이디0번째 이름1번째 키6번째 
        # print('idx3', idx3)
        ...
        for inStr in inFp:
            ...
            if int(row_list[idx3]) >= 165: # idx3(키)가 165와 같거나 작으면 True
